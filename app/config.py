...
class Settings(BaseSettings):
    ...
    gcs_bucket_name: str = Field(
        default="telecom-support-knowledge-base",
        env="GCS_BUCKET_NAME",
    )
    ...
    db_command_timeout_seconds: int = Field(
        default=10,
        env="DB_COMMAND_TIMEOUT_SECONDS",
    )
    ...
    vertex_index_endpoint_id: str = Field(
        default="",
        env="VERTEX_INDEX_ENDPOINT_ID",
    )

    vertex_deployed_index_id: str = Field(
        default="",
        env="VERTEX_DEPLOYED_INDEX_ID",
    )
